...
namespace YourBot
{
    ...
    {
        public static string Decrypt(string data, string encrypKey)
        {
            // 在 encrypKey 右侧填充 \0 到 32 位
            encrypKey = encrypKey.PadRight(32, '\0');

            // 用 base64 解析原密文
            var orginCipher = Encoding.UTF8.GetString(Convert.FromBase64String(data));
            // 取前 16 位为 iv，16 位后的文本为新密文
            var iv = orginCipher.Substring(0, 16);
            var newCipher = orginCipher.Substring(16);
            ...
            {
                aes.Key = Encoding.UTF8.GetBytes(encrypKey);
                aes.IV = Encoding.UTF8.GetBytes(iv);

                ICryptoTransform decryptor = aes.CreateDecryptor(aes.Key, aes.IV);

                using (var memeoryStream = new MemoryStream(newCipherByte))
                using (var csDecrypt = new CryptoStream(memeoryStream, decryptor, CryptoStreamMode.Read))
